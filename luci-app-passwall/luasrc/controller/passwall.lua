...
function index()
    ...
    entry({"admin", "vpn", "passwall", "node_list"}, cbi("passwall/node_list", {autoapply = true}), _("Node List"), 2).dependent = true
    ...
    entry({"admin", "vpn", "passwall", "rule_list"}, cbi("passwall/rule_list"), _("Set Blacklist And Whitelist"), 98).leaf = true
...
function status()
    ...
    for i = 1, tcp_node_num, 1 do
        local listen_port = luci.sys.exec(string.format("[ -f '/var/etc/passwall/port/TCP_%s' ] && echo -n `cat /var/etc/passwall/port/TCP_%s`", i, i))
        e["tcp_node%s_status" % i] = luci.sys.call(string.format("ps -w | grep -v grep | grep -i -E '%s/TCP_%s|brook tproxy -l 0.0.0.0:%s|ipt2socks -T -l %s' >/dev/null",appname, i, listen_port,listen_port)) == 0
    end

    local udp_node_num = luci.sys.exec("echo -n `uci -q get %s.@global_other[0].udp_node_num`" % appname)
    for i = 1, udp_node_num, 1 do
        local listen_port = luci.sys.exec(string.format("[ -f '/var/etc/passwall/port/UDP_%s' ] && echo -n `cat /var/etc/passwall/port/UDP_%s`", i, i))
    ...
    local socks5_node_num = luci.sys.exec("echo -n `uci -q get %s.@global_other[0].socks5_node_num`" % appname)
...
function ping_node()
    ...
    local e = {}
    e.index = index
    if luci.sys.exec("echo -n `uci -q get %s.@global_other[0].use_tcping`" % appname) == "1" and
        luci.sys.exec("echo -n `command -v tcping`") ~= "" then
        e.ping = luci.sys.exec("echo -n `tcping -q -c 1 -i 1 -p " .. port .. " " ..address .. " 2>&1 | grep -o 'time=[0-9]*' | awk -F '=' '{print$2}'`")
    else
        e.ping = luci.sys.exec("echo -n `ping -c 1 -W 1 %q 2>&1 | grep -o 'time=[0-9]*' | awk -F '=' '{print$2}'`" % address)
    end
    luci.http.prepare_content("application/json")
    luci.http.write_json(e)
end
